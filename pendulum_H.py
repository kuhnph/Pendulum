...
def update(i):
    ...
    artists1 = ani.animate(state1, T, fig, ax)
    artists2 = ani2.animate(state2, T, fig, ax)
    artists3 = ani3.animate(state3, T, fig, ax)
    artists4 = ani4.animate(state4, T, fig, ax)
    return *artists1, *artists2, *artists3, *artists4

    #if i > 1: ani.make([artists1, artists2, artists3, artists4], fig, ax, bg)
    #time.sleep(.5)
    #print(f"run time = {format(time.time()-T_start, '2f')}")
funky_anim = animate.FuncAnimation(fig, update,
                            frames = int(T_total/Ts),
                            interval = 1,
                            blit = True)
funky_anim.save("quad_pendulum.gif", 'pillow_writer', fps=30)